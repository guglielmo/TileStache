...
def _get_features(coord, properties, projection, layer, clipped, projected, spacing, simplify):
    ...
    for feature in layer:
        ...
        if simplify is not None and geometry:
            ...
            xspan, yspan = xmax - xmin, ymax - ymin
            logger.debug('TileStache xspan %s yspan %s' % (xspan, yspan))
...
class Provider:
    ...
    def __init__(self, layer, driver, parameters, clipped, verbose, projected, spacing, properties, simplify):
        self.layer = layer
        self.driver = driver
    ...
    def renderTile(self, width, height, srs, coord):
        ...
        features = _get_features(coord, self.properties, self.layer.projection, layer, self.clipped, self.projected, self.spacing, self.simplify)
